    ...
    if st.button("Estimate sponsor ROI"):
        with st.spinner("Loading sponsor metrics..."):
            try:
                ...
                cols = st.columns(4)
                cols[0].metric("Exposure", roi.get("exposure_score", "n/a"))
                cols[1].metric("ROI", f"{roi.get('estimated_roi_pct', 'n/a')}%")
                cols[2].metric("Mentions", roi.get("social_mentions", {}).get("total_mentions", "n/a"))
                cols[3].metric("Sentiment", roi.get("sponsor_sentiment", {}).get("label", "n/a"))
                st.warning(roi.get("do_not_misrepresent", "Simulation output."))
                st.json(roi)
            except requests.RequestException as exc:
                st.error(f"API request failed: {exc}")
        ...
        with st.spinner("Building balanced debate..."):
            try:
                ...
                with col_a:
                    ...
                    for item in debate.get("balanced_arguments", {}).get("for_a", []):
                        st.write(item)
                with col_b:
                    st.markdown(f"#### {debate.get('side_b', 'Side B')}")
                    for item in debate.get("balanced_arguments", {}).get("for_b", []):
                        st.write(item)
                st.info(debate.get("final_judge", {}).get("verdict", "No verdict returned."))
                st.json(debate)
            except requests.RequestException as exc:
                st.error(f"API request failed: {exc}")
    ...
    try:
        ...
        if not metrics.empty:
            ...
            fig = px.bar(
                ...
                y="exposure_score",
                color="sentiment",
                title="Sponsor exposure by brand",
            )
